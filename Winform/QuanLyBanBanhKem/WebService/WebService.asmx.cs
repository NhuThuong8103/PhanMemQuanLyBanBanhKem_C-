...
namespace WebService
{
    ...
    public class WebService : System.Web.Services.WebService
    {
        ...
        #region
        [WebMethod]
        public List<ModelASP.web_DanhSachBanhKem> getDanhSachBanhKemNoiBat()
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                var r = new Random();
                var query = (
                    from a in data.BanhKems
                    from b in data.SizeBanhs
                    from c in data.GiamGias
                    where a.MaBanhKem == b.MaBanhKem
                    where a.MaGiamGia == c.MaGiamGia
                    where b.Gia == (
                        from d in data.SizeBanhs
                        where d.MaBanhKem == a.MaBanhKem
                        select d.Gia).Min()
                    select new ModelASP.web_DanhSachBanhKem
                    {
                        maBanhKem = a.MaBanhKem,
                        maSizeBanh = b.MaSizeBanh,
                        tenBanhKem = a.TenBanhKem,
                        hinhAnhBanhKem = a.HinhAnh,
                        gia = b.Gia,
                        giamGia = int.Parse(c.TenGiamGia),
                        maLoai = a.MaLoaiBanh
                    });
                //return query.OrderBy(x=>r.Next()).Take(4).ToList();
                return query.Skip(r.Next(query.Count())).Take(4).ToList();
            }
        }

        [WebMethod]
        public List<ModelASP.web_DanhSachBanhKem> getDanhSachBanhKemBanChay()
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                ...
                var query = (
                    ...
                    where b.Gia == (
                        ...
                        where d.MaBanhKem == a.MaBanhKem
                        select d.Gia).Min()
                    orderby a.TenBanhKem descending
                    select new ModelASP.web_DanhSachBanhKem
                    {
                        maBanhKem = a.MaBanhKem,
                        maSizeBanh = b.MaSizeBanh,
                ...
                //return query.OrderBy(x=>r.Next()).Take(4).ToList();
                return query.Skip(r.Next(query.Count())).Take(10).ToList();
            }
        }

        [WebMethod]
        public List<ModelASP.web_DanhSachBanhKem> getDanhSachBanhKem_Website()
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
        ...
        [WebMethod]
        public List<ModelASP.web_DanhSachBanhKem> getDanhSachBanhKemChiTiet(int matheloai)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
        ...
        [WebMethod]
        public List<ModelASP.web_DanhSachBanhKem> getDanhSachBanhKemHot()
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
        ...
        [WebMethod]
        public List<ModelASP.web_DanhSachBanhKem> getDanhSachBanhKemTheoTheLoai(int maloaibanh)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
        ...
        [WebMethod]
        public List<TheLoaiBanh> getListTheLoaiBanh_Website()
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                return (from a in data.TheLoaiBanhs
                        select a).ToList();
        ...
        [WebMethod]
        public ModelASP.web_ChiTietBanhKem getTTBanhKem_Website(int mabanh)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;

                return (from a in data.SizeBanhs
                        from b in data.BanhKems
                        from c in data.Sizes
                        from d in data.GiamGias
                        where a.MaBanhKem == b.MaBanhKem
                        where c.MaSize == a.MaSize
                        where d.MaGiamGia == b.MaGiamGia
                        where b.MaBanhKem==mabanh
                        where a.Gia == (
                            from e in data.SizeBanhs where e.MaBanhKem == mabanh select e.Gia
                        ).Min()
                        select new ModelASP.web_ChiTietBanhKem
                        {
                            MaBanhKem=a.MaBanhKem,
                            MaSizeBanh = a.MaSizeBanh,
                            MaSize = c.MaSize,
                            Gia = a.Gia,
                            TenBanhKem = b.TenBanhKem,
                            HinhAnh = b.HinhAnh,
                            MoTa = b.MoTa,
                            TenGiamGia = d.TenGiamGia
                        }).SingleOrDefault();
        ...
        [WebMethod]
        public ModelASP.ChiTietGioHang getTTGioHang_Website(int mabanh)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                ...
                return (from a in data.SizeBanhs
                        ...
                        where c.MaSize == a.MaSize
                        where d.MaGiamGia == b.MaGiamGia
                        where a.MaSizeBanh == mabanh
                        select new ModelASP.ChiTietGioHang
                        {
                            hinhAnhSP=b.HinhAnh,
                            tenSP=b.TenBanhKem,
                            donGia=a.Gia,
                            giamGia=d.TenGiamGia,
                        }).SingleOrDefault();
            }
        }
        ...
        [WebMethod]
        public List<Size> getSize_TTBanhKem(int mabanh)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                ...
                return (from a in data.SizeBanhs
                        from b in data.Sizes
                        from c in data.BanhKems
                        where b.MaSize == a.MaSize
                        where c.MaBanhKem == a.MaBanhKem
                        where c.MaBanhKem == mabanh
                        select b).ToList();
            }
        }
        ...
        public bool checkUserLogin(string email, string password)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                ...
                int count = (from a in data.KhachHangs
        ...
        [WebMethod]
        public void registerAccountUser(string name, string address, string phone, string email, string password)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                ...
                kh.Username = email;
                kh.SDT_KhachHang = phone;
                kh.DiaChi_KhachHang = address;
        ...
        [WebMethod]
        public ModelASP.GiaTheoSize getGiaTheoSize(string idbanh, string idsize)
        {
            using(DatabaseDataContext data= new DatabaseDataContext()){
                data.DeferredLoadingEnabled = false;
                return (from a in data.BanhKems
                        ...
                        where a.MaBanhKem == b.MaBanhKem
                        where a.MaGiamGia == c.MaGiamGia
                        where a.MaBanhKem == int.Parse(idbanh)
                        where b.MaSize == int.Parse(idsize)
                        select new ModelASP.GiaTheoSize
                        {
                            gia=b.Gia,
                            giamgia=c.TenGiamGia
                        }).FirstOrDefault();
            }
        }
        ...
        public List<Quyen> loadQuyen()
        {
            ...
            {
                data.DeferredLoadingEnabled = false;
                var query = from q in data.Quyens select q;
                return query.ToList();
            }
        }
        [WebMethod]
        public void ThemNhanVienQuyen(string hoten, string gt, string sdt, string diachi, string username, string password)
        {
            using (DatabaseDataContext data = new DatabaseDataContext())
            {
                data.DeferredLoadingEnabled = false;
                ...
                //NhanVien_Quyen nvq = new NhanVien_Quyen();
                //var query = from nvv in data.NhanViens select get_MAXMaHoaDon( nvv.MaN)
                //nvq.MaNhanVien = 
